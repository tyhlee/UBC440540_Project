...
#method: BD2, MBD
custom_fbplot <- function(fit,x=NULL,method='MBD',depth=NULL,plot=TRUE,prob=0.5,color=6,outliercol=2,
  ...
  return(data.frame(y=y,lower=yy[1:length(y)],upper=rev(yy[(length(y)+1):(length(y)*2)])))
}
